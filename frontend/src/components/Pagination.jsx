...
export default function Pagination({ 
  ...
  return (
    <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-4 rounded-lg border transition-all duration-300 ${
      ...
      {/* Records Info */}
      <div className={`text-sm ${isDark ? 'text-silver-400' : 'text-gray-600'}`}>
      ...
      <div className="flex items-center gap-2">
        <label className={`text-sm font-medium ${isDark ? 'text-silver-300' : 'text-gray-700'}`}>
          Records per page:
        </label>
        <select
          value={pageSize}
      ...
      {/* Page Info */}
      <div className={`text-sm font-medium ${isDark ? 'text-silver-300' : 'text-gray-700'}`}>
